...
#include "KNeighbourOrderedForwarding.h"
#include "inet/common/ModuleAccess.h"
#include <math.h>

Define_Module(KNeighbourOrderedForwarding);

void KNeighbourOrderedForwarding::initialize(int stage)
{
    if (stage == 0) {
        // get parameters
    ...
    //std::cout << "stage 1 " << "\n" ;
        ...
        cacheSizeReportingTimeoutEvent->setKind(KNEIGHBOURORDEREDFORWARDING_CACHESIZE_REP_EVENT);
        ...
        EV_FATAL << KNEIGHBOURORDEREDFORWARDING_SIMMODULEINFO << "Something is radically wrong in initialisation \n";
    }
}

int KNeighbourOrderedForwarding::numInitStages() const
{
    return 3;
}

void KNeighbourOrderedForwarding::handleMessage(cMessage *msg)
{
    cGate *gate;
    char gateName[64];
    ...
    if (msg->isSelfMessage()) {
        if (msg->getKind() == KNEIGHBOURORDEREDFORWARDING_CACHESIZE_REP_EVENT) {
            ...
            EV_INFO << KNEIGHBOURORDEREDFORWARDING_SIMMODULEINFO << "Received unexpected self message" << "\n";
            delete msg;
        }
    ...
    // messages from other layers
       ...
       // neighbour list message arrived from the lower layer (link layer)
        ...
        // received some unexpected packet
            ...
            EV_INFO << KNEIGHBOURORDEREDFORWARDING_SIMMODULEINFO << "Received unexpected packet" << "\n";
            delete msg;
        }
    }
}

void KNeighbourOrderedForwarding::ageDataInCache()
{
...
void KNeighbourOrderedForwarding::handleAppRegistrationMsg(cMessage *msg)
{
...
void KNeighbourOrderedForwarding::handleDataMsgFromUpperLayer(cMessage *msg)
{
...
void KNeighbourOrderedForwarding::handleNeighbourListMsgFromLowerLayer(cMessage *msg)
{
    ...
    for (int i = 0; i < angleAndRSSIAndSpeedOrderList.size(); i++) {
        string nodeMACAddress = neighListMsg->getNeighbourNameList(angleAndRSSIAndSpeedOrderList[i]->neighIndex);
        ...
        if (syncWithNeighbour) {
            ...
            //cout << KNEIGHBOURORDEREDFORWARDING_SIMMODULEINFO << "neigh Mac" << nodeMACAddress << "\n";
...
void KNeighbourOrderedForwarding::handleDataMsgFromLowerLayer(cMessage *msg)
{
    KDataMsg *omnetDataMsg = dynamic_cast<KDataMsg*>(msg);
    bool found;
...
void KNeighbourOrderedForwarding::handleSummaryVectorMsgFromLowerLayer(cMessage *msg)
{
    ...
    int realPacketSize = 6 + 6 + (selectedMessageIDList.size() * KNEIGHBOURORDEREDFORWARDING_MSG_ID_HASH_SIZE);
...
void KNeighbourOrderedForwarding::handleDataRequestMsgFromLowerLayer(cMessage *msg)
{
...
KNeighbourOrderedForwarding::SyncedNeighbour* KNeighbourOrderedForwarding::getSyncingNeighbourInfo(string nodeMACAddress)
{
...
void KNeighbourOrderedForwarding::setSyncingNeighbourInfoForNextRound()
{
...
void KNeighbourOrderedForwarding::setSyncingNeighbourInfoForNoNeighboursOrEmptyCache()
{
    // loop thru syncing neighbor list and set for next round
    list<SyncedNeighbour*>::iterator iteratorSyncedNeighbour;
    iteratorSyncedNeighbour = syncedNeighbourList.begin();
...
KSummaryVectorMsg* KNeighbourOrderedForwarding::makeSummaryVectorMessage()
{
...
bool KNeighbourOrderedForwarding::isAngleBetween(double angle, double lowerBoundary, double upperBoundary)
{
...
void KNeighbourOrderedForwarding::finish()
{
...
>>>>>>> dadab56a0bcb16405cd20281df7c3f3281824588
