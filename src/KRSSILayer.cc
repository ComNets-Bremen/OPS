...
#include "KRSSILayer.h"
#include "inet/common/ModuleAccess.h"
#include <math.h>

Define_Module(KRSSILayer);

void KRSSILayer::initialize(int stage)
{
    if (stage == 0) {
        // get parameters
    ...
    //std::cout << "stage 1 " << "\n" ;
        ...
        cacheSizeReportingTimeoutEvent->setKind(KRSSILAYER_CACHESIZE_REP_EVENT);
        ...
        EV_FATAL << KRSSILAYER_SIMMODULEINFO << "Something is radically wrong in initialisation \n";
    }
}

int KRSSILayer::numInitStages() const
{
    return 3;
}

void KRSSILayer::handleMessage(cMessage *msg)
{
    cGate *gate;
    char gateName[64];
    ...
    if (msg->isSelfMessage()) {
        if (msg->getKind() == KRSSILAYER_CACHESIZE_REP_EVENT) {
            ...
            EV_INFO << KRSSILAYER_SIMMODULEINFO << "Received unexpected self message" << "\n";
            delete msg;
        }
    ...
    // messages from other layers
       ...
       // neighbour list message arrived from the lower layer (link layer)
        ...
        // received some unexpected packet
            ...
            EV_INFO << KRSSILAYER_SIMMODULEINFO << "Received unexpected packet" << "\n";
            delete msg;
        }
    }
}

void KRSSILayer::ageDataInCache()
{
...
void KRSSILayer::handleAppRegistrationMsg(cMessage *msg)
{
...
void KRSSILayer::handleDataMsgFromUpperLayer(cMessage *msg)
{
...
void KRSSILayer::handleNeighbourListMsgFromLowerLayer(cMessage *msg)
{
...
//            }
    ...
    // get RSSI/ distance list for node in neighbor list
    for (int x = 1; x <=5; x++) {
        // x = 1
        int j = 0;
        while (j < neighListMsg->getNeighbourNameListArraySize()) {
            if (neighListMsg->getNeighbourRangeCatList(j) == x){
            //if (priorityList[j] == selectedPriorityCodeList[x]) {
                ...
                if (syncWithNeighbour) {
                    ...
                    //cout << KRSSILAYER_SIMMODULEINFO << "neigh Mac" << nodeMACAddress << "\n";
...
void KRSSILayer::handleDataMsgFromLowerLayer(cMessage *msg)
{
    KDataMsg *omnetDataMsg = dynamic_cast<KDataMsg*>(msg);
    bool found;
...
void KRSSILayer::handleSummaryVectorMsgFromLowerLayer(cMessage *msg)
{
    ...
    int realPacketSize = 6 + 6 + (selectedMessageIDList.size() * KRSSILAYER_MSG_ID_HASH_SIZE);
...
void KRSSILayer::handleDataRequestMsgFromLowerLayer(cMessage *msg)
{
...
KRSSILayer::SyncedNeighbour* KRSSILayer::getSyncingNeighbourInfo(string nodeMACAddress)
{
...
void KRSSILayer::setSyncingNeighbourInfoForNextRound()
{
...
void KRSSILayer::setSyncingNeighbourInfoForNoNeighboursOrEmptyCache()
{
    // loop thru syncing neighbor list and set for next round
    list<SyncedNeighbour*>::iterator iteratorSyncedNeighbour;
    iteratorSyncedNeighbour = syncedNeighbourList.begin();
...
KSummaryVectorMsg* KRSSILayer::makeSummaryVectorMessage()
{
...
bool KRSSILayer::isAngleBetween(double angle, double lowerBoundary, double upperBoundary)
{
