...
- Create the input data files to UBM and ExtendedSWIM: run `python ./tools/ubm-data-gen.py ./tools/sample-ubm-data-gen-params.txt`. This will create `locations.txt`, `events.txt` and `properties.txt` files. For mor in[Check FORMATS](./FORMATS.md)
...
- Once a simulation is run (using `ops-simu-run.sh`), the simulation will generate a log file 
with a name starting with `General...` such as  `General-0-20170822-12:23:43-56750-log2.txt` 
and a set of parser output files having the same prefix. The `parsers.txt` lists the 
`...stats.py` parsers that have to be run.

- Use a text editor to view the files created by the parsers (above) and use the data in these
parser output files to modify the required `...plot.py` files.
